...
def build_system_prompt(
    ...
    Args:
        ...
        flame_on: Whether Flame On mode is active (answers from working-process data)
    """
    parts = [FENIX_BASE_PROMPT]

    # Flame On mode adjustments
    if flame_on:
        parts.append(
            "\n## FLAME ON MODE (Active)\n"
            "The visitor has activated Flame On mode. You are now answering from Kiran's "
            "working-process data — daily journal entries, session transcripts, product guides, "
            "and connecting threads essays. This is the raw, unpolished view of how Kiran "
            "actually works, thinks, and builds.\n\n"
            "IMPORTANT RULES for Flame On:\n"
            "- Answer ONLY from the knowledge base below. Do NOT use published portfolio content.\n"
            "- Be transparent about the source: 'From Kiran's journal on [date]...' or "
            "'In a build session from [date]...'\n"
            "- If you don't have information to answer a question, say: "
            "'I'm constantly learning and building my understanding of Kiran's working process. "
            "I don't have clarity on that yet, but I'll track it as something I need to explore.'\n"
            "- NEVER fabricate answers. Honest gaps are better than hollow responses.\n"
            "- You can share insights about patterns, decision-making, failures, and growth — "
            "these are the things that make Flame On valuable.\n"
            "- The tone should be more reflective and candid than the polished portfolio voice."
        )
    else:
        # Persona-specific framing (only in published mode)
    ...
    if rag_context_text:
        ...
        if flame_on:
            parts.append(
                "\n## Working-Process Data\nNo relevant working-process data was found for this query. "
                "I'm constantly learning and building my understanding of Kiran's working process. "
                "I don't have clarity on this topic yet, but I'll track it as something I need to explore. "
                "You might try asking about Kiran's build process, decision-making patterns, "
                "what he's been working on recently, or how specific features were built."
            )
        else:
            parts.append(
...
async def get_or_create_conversation(
    ...
    if session_id:
        ...
        if result.data:
            ...
            # Update last_active_at (and page_url if provided — visitor may navigate)
            update_data = {"last_active_at": datetime.now(timezone.utc).isoformat()}
            if page_url:
                update_data["page_url"] = page_url
            db.table("conversations").update(update_data).eq("id", conv_id).execute()
...
async def store_message(
    ...
    # Add search metadata (only for assistant messages)
    if rag_chunks_used is not None:
        msg_data["rag_chunks_used"] = rag_chunks_used
    if similarity_scores is not None:
        msg_data["similarity_scores"] = similarity_scores
    if search_type is not None:
        msg_data["search_type"] = search_type
...
async def chat_stream(
    ...
    Flow:
    1. Get/create conversation (with page_url + user_agent)
    2. Infer persona from message + history
    3. RAG retrieval for relevant content + training data
    4. Build system prompt (base + persona + RAG context + nudge)
    5. Stream Claude response
    6. Store messages with search metadata
    7. Log low-confidence questions to training queue

    Args:
        flame_on: When True, RAG searches ONLY 'flame_on' source data
                 (journal entries, session transcripts, product guides).
                 When False, searches ONLY 'published' curated content.
    ...
    # 1. Get or create conversation (now logs page_url and user_agent)
    try:
        conversation_id, history = await get_or_create_conversation(
            session_id,
            page_url=page_url,
    ...
    # 3. RAG retrieval (now includes training_data and search metadata)
    ...
    # Determine source_type filter based on Flame On toggle
    source_type = "flame_on" if flame_on else "published"
    ...
    system_prompt = build_system_prompt(persona, rag_context_text, page_context, flame_on=flame_on)
    ...
    # 9. Store assistant response (with search metadata)
    await store_message(
        conversation_id,
        "assistant",
        full_response,

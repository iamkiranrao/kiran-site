...
(function () {
  ...
  function buildUnlockCards(container) {
    ...
    var cards = [
      ...
      },
      {
        ...
        tag: 'From me to you',
        hook: 'I\'ve spent enough time in corporate to know what the break room really needs. These are AI-generated, printable, and funnier than anything HR has approved.<br>You\'re welcome! \uD83D\uDE09',
    ...
    cards.forEach(function (card) {
      ...
      cardEl.appendChild(el('div', 'ev-card-hook', { html: card.hook }));
      if (card.gateReason) {
  ...
  function renderRecruiterQuestionsPanel(panel) {
    ...
    RECRUITER_QUESTIONS.forEach(function (qa, idx) {
      ...
      header.setAttribute('aria-controls', 'ev-acc-body-' + idx);

      var headerLeft = el('div', 'ev-accordion-header-left');
      var chevron = el('span', 'ev-accordion-chevron', {
      ...
      if (qa.leader) {
        header.appendChild(el('span', 'ev-accordion-leader', { text: qa.leader }));
      }

      item.appendChild(header);

      var body = el('div', 'ev-accordion-body');
      body.setAttribute('id', 'ev-acc-body-' + idx);
      var bodyInner = el('div', 'ev-accordion-body-inner');
      if (qa.anecdote) {
        bodyInner.appendChild(el('div', 'ev-question-anecdote', { text: qa.anecdote }));
      }
